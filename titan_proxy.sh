...
function install_node() {
    ...
    echo -e "${CLR_INFO}Запускаем контейнер Titan на порту 45000...${CLR_RESET}"
    docker run -d -p 45000:80 -v ~/.titanedge:/root/.titanedge nezha123/titan-edge

    echo -e "${CLR_SUCCESS}✅ Установка завершена! Нода запущена на порту 45000.${CLR_RESET}"
}
...
function change_port() {
    echo -e "${CLR_INFO}Введите новый порт для ноды (рекомендуем 45000 или 50050):${CLR_RESET}"
    read -r NEW_PORT

    echo -e "${CLR_INFO}Останавливаем текущий контейнер...${CLR_RESET}"
    ...
    echo -e "${CLR_INFO}Запускаем контейнер с новым портом ${NEW_PORT}...${CLR_RESET}"
    docker run -d -p ${NEW_PORT}:80 -v ~/.titanedge:/root/.titanedge nezha123/titan-edge

    echo -e "${CLR_SUCCESS}✅ Порт успешно изменен! Нода теперь работает на порту ${NEW_PORT}.${CLR_RESET}"
}
...
function show_menu() {
    ...
    echo -e "${CLR_GREEN}5) 🔄 Сменить порт ноды${CLR_RESET}"
